    ...
    try {
      ...
      .sort({ createdAt: -1 });
       ...
       shows.forEach((show)=>{
        ...
        // below if stmt is executed only for unique theatres
        if(!theatre){
            // showsForTheTheatre array contains shows that are played 
            // in a particular theatre 
            const showsForTheTheatre = shows.filter((showObj)=>{
                return showObj.theatre._id === show.theatre._id;
            })
            /*
                uniqueTheatres array contains theatre obj details
                and shows which are being played in the theatre
            */
            uniqueTheatres.push({
                ...show.theatre._doc,
                shows:showsForTheTheatre
            })
        }
       })
       res.send({
